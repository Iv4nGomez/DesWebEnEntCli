...
function aceptarAltaArbol() {
  ...
  const tallaje = document.getElementById("txtTallaje")
  const especie = document.getElementById("txtEspecie")

  const caduco = document.getElementById("rbtTipoArbol-C")

  const esFrutal = document.getElementById("rbtFrutal-S")
  const mesFloracion = document.getElementById("txtMesFloracion")

  if (codigo.value == "" || tallaje.value == "" || especie.value == "") {
    alert("No se han rellenado todos los datos")
    return
  }

  if (caduco.checked) {
      if (mesFloracion.value == "") {
    alert("No se han rellenado todos los datos")
    return
  }
    oArbol = new Caduco(parseInt(codigo.value), parseInt(tallaje.value), especie.value, mesFloracion.value)
  } else {
    oArbol = new Perenne(parseInt(codigo.value), parseInt(tallaje.value), especie.value, esFrutal.checked)
  }
...
function aceptarTallaje() {
    ...
    const codigo = document.getElementById("txtCodigoArbol");
    const tallaje = document.getElementById("txtTallajeArbol");

    if(codigo.value == "" || tallaje.value == "") {
      alert("Rellena todos los datos")
      return
    }

  
  let sRespuesta = oVivero.tallajeArbol(parseInt(codigo.value), parseInt(tallaje.value));
